# 🛒 이커머스 한정판 상품 선점 및 결제 시스템 (동시성 제어)

> **트래픽이 집중되는 한정판 상품 선점 시 발생하는 재고 동시성 문제(Race Condition)를 비관적 락(Pessimistic Lock)을 통해 해결하고 데이터 무결성을 보장하는 프로젝트입니다.**
...
### 🗄️ 데이터베이스 ERD 구조 (`order_info` & `goods`)
```sql
-- 1. 상품 테이블 (goods)
